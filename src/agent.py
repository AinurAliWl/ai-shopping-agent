        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Standalone product search query. "
                        "For follow-up questions, preserve relevant requirements and "
                        "constraints from the conversation history."
                    },
                    "max_price": {
                        "type": "number",
...
27. Never invent missing product information. If a required fact is not
present in the tool results, state that it is not available.
...
31. If the user specifies a delivery deadline but the tools provide
only warehouse-to-store transfer information, explicitly state that
customer delivery time cannot be verified from the available data.
...
You may only state the recorded value itself.
...
37. Do not describe one product component as newer, older,
higher-generation, lower-generation, or from a different generation
than another component unless this comparison is explicitly provided
by a tool result.
...
contains 1–2 days for Astana Mega Silk Way and 2–3 days for Astana Khan
Shatyr, the final answer must contain exactly those two separate route
estimates and must never contain "1–3 days".
...
were found in the available catalog or inventory. Do not claim that
such a product does not exist in the real world, does not exist
generally, or cannot be found elsewhere.

58. Do not repeat the full product list or full product specifications
in the final answer when verified_products are returned. Keep the final
answer concise and use it only for a short summary of the search result,
requirements, and relevant availability information.

59. Use conversation history to resolve follow-up questions such as
"which one", "the first one", "among these", "what about the SSD", or
similar references to previously discussed products.

60. When calling search_products for a follow-up question, construct a
standalone search query that preserves relevant requirements and product
constraints from the conversation history.

61. Do not discard previously established product constraints when
interpreting a follow-up question unless the user explicitly changes
them.

62. Do not expose internal catalog fields such as ram_expandable,
ram_max_gb, product_id, checked_candidates, or other internal database
fields unless the user explicitly asks for them. Present only the
user-facing product specifications provided by the tool results.
"""
...
def run_agent(user_query, chat_history=None):
    ...
    while True:
        ...
        if not response.choices:
            return {
                "answer": "ERROR: OpenRouter returned no choices.",
                "verified_products": [],
                "verified_count": 0,
            }
        ...
        if not message.tool_calls:
            ...
            return {
                "answer": final_answer,
                "verified_products": (
                    verified_result["verified_products"]
                    if verified_result
                    else []
                ),
                "verified_count": (
                    verified_result["verified_count"]
                    if verified_result
                    else 0
                ),
            }
        ...
        for tool_call in message.tool_calls:
            ...
            if tool_call.function.name == "search_products":
                ...
                if search_called:
                    ...
                    destination_store = detect_destination_store(
                        user_query
                    )
